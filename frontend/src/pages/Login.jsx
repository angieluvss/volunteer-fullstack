...
const Login = ({ setToken, setRole, setVolunteerFormCompleted }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const volunteerFormStatus = await fetchProfileStatus(setRole, setVolunteerFormCompleted);
  
      // Decide where to navigate based on the role and form status
      if (role === 'admin') {
        navigate('/admin-dashboard');
      } else if (role === 'volunteer') {
        if (volunteerFormStatus === 'not completed') {
